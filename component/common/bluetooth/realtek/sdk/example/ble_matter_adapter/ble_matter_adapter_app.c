...
bool matter_multi_adv_start_by_id(uint8_t *adv_id, uint8_t *adv_data, uint16_t adv_len, uint8_t *rsp_data, uint16_t rsp_len, uint8_t type)
{
	...
	if (h_adv_param->one_shot_timer == NULL) {
		if (os_timer_create(&h_adv_param->one_shot_timer, "start adv_timer", adv_index, (int)(h_adv_param->H_adv_intval * 0.625), 1,
							ble_matter_adapter_legacy_start_adv_callback) == false) {
...
void ble_matter_adapter_multi_adv_task_func(void *arg)
{
	...
	while (true) {
		if (os_msg_recv(matter_multi_adapter.queue_handle, &adv_index, 0xFFFFFFFF) == true) {
			...
			le_adv_set_param(GAP_PARAM_ADV_LOCAL_ADDR_TYPE, sizeof(matter_multi_adv_param_array[adv_index].local_bd_type),
							 &matter_multi_adv_param_array[adv_index].local_bd_type);
			if (matter_multi_adv_param_array[adv_index].update_adv_mutex) {
				...
				if (matter_multi_adv_param_array[adv_index].type == 2) { //customer
					le_adv_set_param(GAP_PARAM_SCAN_RSP_DATA, matter_multi_adv_param_array[adv_index].scanrsp_datalen,
									 (void *)matter_multi_adv_param_array[adv_index].scanrsp_data);
...
void ble_matter_adapter_multi_adv_init()
{
	...
	if (os_task_create(&matter_multi_adapter.task_handle, "multi_adv_task", ble_matter_adapter_multi_adv_task_func, NULL, MATTER_MULTI_TASK_STACK_SIZE,
					   MATTER_MULTI_TASK_PRIORITY) == false) {
...
void ble_matter_adapter_app_handle_conn_state_evt(uint8_t conn_id, T_GAP_CONN_STATE new_state, uint16_t disc_cause)
{
	...
	case GAP_CONN_STATE_DISCONNECTED: {
		...
		if (link_customer == 1) { //matter
			...
			matter_multi_adv_start_by_id(&customer_adv_id, customer_adv_data, customer_adv_data_length, customer_rsp_data, customer_rsp_data_length,
