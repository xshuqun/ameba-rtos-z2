...
void ssl_client_thread(char *server_host)
{
	if (xTaskCreate(ssl_client_handler, "ssl_client_handler", 2048, server_host, tskIDLE_PRIORITY + 3 + PRIORITY_OFFSET, NULL) != pdPASS) {
		...
		if (select(max_socket_fd + 1, &read_fds, NULL, NULL, &timeout)) {
			for (socket_fd = min_socket_fd; socket_fd < max_socket_fd + 1; socket_fd ++) {
				if (socket_used[socket_fd] && FD_ISSET(socket_fd, &read_fds)) {
					int read_size = recvfrom(socket_fd, buf, sizeof(buf), 0, (struct sockaddr *) &client_addr, (socklen_t *)&addrlen);
					if (read_size > 0) {
...
#endif //#if defined(CONFIG_EXAMPLE_HIGH_LOAD_MEMORY_USE) && CONFIG_EXAMPLE_HIGH_LOAD_MEMORY_USE